...
{
	[Desc("Actors possessing this trait should define the {0} trait. When the prerequisites are fulfilled, ",
		"this trait grants a level-up to newly spawned actors. If additionally the actor's owning player defines the {1} ",
		"trait, the production queue icon renders with an overlay defined in that trait.")]
	[DescArg(typeof(GainsExperienceInfo))]
	[DescArg(typeof(Render.VeteranProductionIconOverlayInfo))]
